 ...
 */
export function createKV(namespace, rateLimitPrefix) {
 ...
 */
export function createRateLimitKV(namespace) {
    const resolvedNamespace = namespace ?? env.KV_NAMESPACE;
    ...
    if (!redisUrl) {
        ...
        throw new Error('Redis URL for rate limiting is not configured. Please set RATE_LIMIT_REDIS_URL or KV_URL environment variable.');
    }
