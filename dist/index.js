...
export class KV {
    ...
    rateLimitPrefix;
    #keyv;
    #redisClient; // Dedicated Redis client for rate limiter
    #limiterInstances = new Map();
    #isRateLimiterReady = false;
    constructor(redisUrl, namespace, // Keyv namespace
    rateLimitPrefix = 'rl') {
        ...
        this.#keyv.on('error', err => {
            ...
            const isTransientSocketClose = err instanceof Error && err.name === 'SocketClosedUnexpectedlyError';
    ...
    getRateLimiter(id, options) {
        ...
        if (this.#isRateLimiterReady && this.#redisClient.status === 'ready') {
            ...
            debugLimiter('[limiter] Redis not ready - creating in-memory rate limiter (id: %s, redisReady: %s, redisStatus: %s)', id, this.#isRateLimiterReady, this.#redisClient.status);
